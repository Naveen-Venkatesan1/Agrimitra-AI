/**
 * Official India States & Union Territories with Districts Dataset
 * Covers all 28 States & 8 Union Territories (36 Administrative Divisions)
 */
...
export const INDIA_LOCATIONS = {
  ...
  "Andhra Pradesh": [
    ...
    "Kakinada", "Konaseema", "Kurnool", "Nandyal", "NTR", "Palnadu", "Parvathipuram Manyam",
  ...
  "Arunachal Pradesh": [
    ...
    "Kurung Kumey", "Lepa Rada", "Lhit", "Longding", "Lower Dibang Valley", "Lower Subansiri",
  ...
  "Sikkim": [
    "Gangtok", "Gyalshing", "Mangan", "Namtchi", "Pakyong", "Soreng"
  ],
  "Tamil Nadu": [
...
/**
 * Get all 36 States & Union Territories sorted alphabetically
 */
export const getStatesAndUTs = () => {
  return Object.keys(INDIA_LOCATIONS).sort();
};

/**
 * Get districts for a specified State or Union Territory
 */
export const getDistrictsByState = (stateName) => {
  if (!stateName || !INDIA_LOCATIONS[stateName]) {
    return INDIA_LOCATIONS["Tamil Nadu"]; // Fallback
  }
  return INDIA_LOCATIONS[stateName];
};

/**
 * Get coordinates & climate profile for any district
 */
export const getLocationCoordinates = (stateName, districtName) => {
  // Mapping baseline coordinates for key agricultural centers
  const coordsMap = {
    "Tamil Nadu": { lat: 10.7870, lon: 79.1378, name: "Thanjavur, Tamil Nadu" },
    ...
    "Gujarat": { lat: 23.0225, lon: 72.5714, name: "Ahmedabad, Gujarat" },
    "Karnataka": { lat: 12.9716, lon: 77.5946, name: "Bengaluru, Karnataka" },
    "West Bengal": { lat: 22.5726, lon: 88.3639, name: "Kolkata, West Bengal" },
    "Telangana": { lat: 17.3850, lon: 78.4867, name: "Hyderabad, Telangana" },
    "Andhra Pradesh": { lat: 16.5062, lon: 80.6480, name: "Vijayawada, Andhra Pradesh" },
    "Haryana": { lat: 29.6857, lon: 76.9905, name: "Karnal, Haryana" },
    "Rajasthan": { lat: 26.9124, lon: 75.7873, name: "Jaipur, Rajasthan" },
    "Bihar": { lat: 25.5941, lon: 85.1376, name: "Patna, Bihar" },
    "Madhya Pradesh": { lat: 23.2599, lon: 77.4126, name: "Bhopal, Madhya Pradesh" }
  };

  if (coordsMap[stateName]) {
    return {
      lat: coordsMap[stateName].lat,
      lon: coordsMap[stateName].lon,
      name: `${districtName || stateName}, ${stateName}`
    };
  }
  ...
  return {
    lat: 10.7870,
    lon: 79.1378,
    name: `${districtName || 'Thanjavur'}, ${stateName || 'Tamil Nadu'}`
  };
};
