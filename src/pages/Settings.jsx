...
export const Settings = () => {
  ...
  return (
    <div className="space-y-6 w-full animate-fade-in pb-10">
      <div>
        <span className="text-xs font-semibold text-agri-light uppercase tracking-wider">{t('preferences', 'Preferences')}</span>
        <h1 className="text-2xl sm:text-3xl font-extrabold text-agri-dark mt-0.5">{t('nav_settings', 'Settings')}</h1>
        <p className="text-xs sm:text-sm text-gray-500 mt-1">Configure language, location defaults, notification channels & farm profile</p>
      </div>
      ...
      {saved && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 text-sm font-semibold flex items-center gap-2">
          ...
          <span>{t('settings_saved_success', 'Settings saved successfully.')}</span>
        </div>
      )}
      ...
      <Card hover={false} className="p-6 space-y-4">
        <h3 className="text-sm font-bold text-agri-dark border-b pb-2 flex items-center gap-2">
          <MapPin className="w-4 h-4 text-agri-primary" /> {t('default_farm_location', 'Default Farm Location (Global State & District)')}
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">{t('filter_state', 'State / Union Territory')}</label>
            <SearchableSelect
              value={selectedState}
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">{t('filter_district', 'District')}</label>
            <SearchableSelect
      ...
      <Card hover={false} className="p-6 space-y-4">
        <h3 className="text-sm font-bold text-agri-dark border-b pb-2 flex items-center gap-2">
          <Globe className="w-4 h-4 text-agri-primary" /> {t('regional_language_preference', 'Regional Language Preference')}
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">{t('app_language', 'Application Language')}</label>
            <select
              value={currentLang}
      ...
      <Card hover={false} className="p-6 space-y-4">
        <h3 className="text-sm font-bold text-agri-dark border-b pb-2 flex items-center gap-2">
          <Bell className="w-4 h-4 text-agri-primary" /> {t('notification_channels', 'Notification Channels')}
        </h3>
        <div className="space-y-3">
          {[
            { key: 'push', title: 'In-App Push Notifications', desc: 'Receive real-time alerts on your phone or web app' },
            { key: 'whatsapp', title: 'WhatsApp Farmer Advisory', desc: 'Receive daily crop reports & weather forecast on WhatsApp' },
            { key: 'sms', title: 'Critical SMS Alerts', desc: 'Receive high-priority sensor & weather alerts via SMS (works offline)' },
          ].map((item) => (
