...
const generateAgriInsights = (currentData, dailyData, wmoInfo, districtName, userCrop) => {
  const temp = safeRound(currentData?.temperature_2m);
  const windSpeed = safeRound(currentData?.wind_speed_10m);
  const rainProb = dailyData?.precipitation_probability_max?.[0] ?? null;
...
const fetchLiveWeather = async (districtName = "Thanjavur", stateName = "Tamil Nadu", userCrop = null, cropStage = null, parentSignal = null) => {
  ...
  try {
    ...
    const weatherSummary = {
      ...
      temp: safeRound(data.current?.temperature_2m),
      feelsLike: safeRound(data.current?.apparent_temperature),
      condition: currentWmo.condition,
      humidity: safeRound(data.current?.relative_humidity_2m),
      windSpeed: data.current?.wind_speed_10m != null ? `${safeRound(data.current.wind_speed_10m)} km/h` : null,
      rainfall: data.current?.precipitation != null ? `${data.current.precipitation} mm` : null,
      pressure: data.current?.surface_pressure != null ? `${safeRound(data.current.surface_pressure)} hPa` : null,
      high: safeRound(data.daily?.temperature_2m_max?.[0]),
      low: safeRound(data.daily?.temperature_2m_min?.[0]),
      rainProbabilityTomorrow: data.daily?.precipitation_probability_max?.[1] ?? null,
      dailyForecast,
    ...
    // Calculate crop intelligence based on raw weather data
    const intelligence = analyzeWeatherForCrop({
      location: `${districtName}, ${stateName}`,
      crop: userCrop,
      growthStage: cropStage,
      temperature: weatherSummary.temp,
      humidity: weatherSummary.humidity,
      rainProbability: weatherSummary.rainProbabilityTomorrow,
      rainfall: weatherSummary.rainfall,
      windSpeedStr: weatherSummary.windSpeed,
      windDirection: 0,
      forecast7Day: weatherSummary.dailyForecast
    });

    weatherSummary.intelligence = intelligence;
