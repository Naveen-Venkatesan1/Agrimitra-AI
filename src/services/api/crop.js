...
Provide the top 3 recommended crops for highest yield. Return JSON array of objects with keys: "crop", "matchScore", "suitability", "waterRequirement", "expectedYield", "season", "duration", "marketDemand". Do not wrap in markdown.`;
  ...
  async analyzeCropDisease(file, language = "English", uid = null) {
    ...
    try {
      ...
      if (data.status === 'ERROR') {
        return { success: false, error: data.error || 'AI Analysis Error' };
      }
      ...
      if (data.success !== false) {
        ...
        // Non-blocking background persistence so diagnosis renders instantly in UI
        (async () => {
          try {
            ...
            console.warn('IndexedDB image save warning:', idbErr);
          }
